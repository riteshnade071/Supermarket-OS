...
GEMINI_MODEL = "gemini-3.6-flash"  # current recommended free-tier Flash model
...
async def ask_owner_assistant(db: Session, store_id: str, question: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            res = await client.post(
                ...
                json={
                    ...
                    "generationConfig": {
                        ...
                        # Gemini 3.x uses thinkingLevel (not the old thinkingBudget field) —
                        # keep it low so token budget goes to the visible answer.
            ...
            if block_reason:
                return f"Ye sawaal answer nahi ho paya (content filter: {block_reason}). Alag tarike se pooch ke dekho."

            candidates = data.get("candidates", [])
            if not candidates:
                return f"Jawab nahi mil paya (no candidates in response: {data})"
            finish_reason = candidates[0].get("finishReason", "?")
            parts = candidates[0].get("content", {}).get("parts", [])
            text = "".join(p.get("text", "") for p in parts)
            return text.strip() or f"Jawab nahi mil paya (empty response, finishReason: {finish_reason})"
    except httpx.HTTPStatusError as e:
        return f"AI assistant error {e.response.status_code}: {e.response.text[:300]}"
    except Exception as e:
        return f"AI assistant abhi jawab nahi de paya — {type(e).__name__}: {str(e)[:200]}"
