...
def dashboard_summary(store_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def actions_required(store_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    ...
    for item in low_stock[:10]:
        message = (
            f"{item['product_name']} stock khatam ho sakta hai {item['days_left']} din mein. "
            f"Suggested purchase: {item['suggested_purchase_qty']} units."
        )
        # Price Intelligence: if a cheaper supplier is on file, say so right in the alert
        if item.get("potential_saving"):
            message += (
                f" 💡 {item['best_supplier_name']} se lo (₹{item['best_supplier_rate']}/unit) "
                f"instead of {item.get('suggested_supplier') or 'current supplier'} — "
                f"saving ₹{item['potential_saving']}."
            )
