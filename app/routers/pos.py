...
from app.models.core import Product, Batch
...
def create_sale(payload: SaleCreate, tenant_id: str, db: Session = Depends(get_db)):
    """
    ...
    for item in payload.items:
        ...
        # FEFO batch pick (optional - only if batches exist)
    ...
    for item_data in sale_items_data:
        ...
        # Deduct stock - THIS is what keeps inventory accurate
...
def cancel_sale(sale_id: str, tenant_id: str, db: Session = Depends(get_db)):
    ...
    for item in items:
        record_stock_movement(
            ...
            change_type="return",
            quantity_change=item.quantity,  # reverse the deduction
            reference_id=sale.id,
            note="Sale cancelled",
        )
...
def list_sales(tenant_id: str, store_id: str, db: Session = Depends(get_db)):
    return (
        db.query(Sale)
        .filter(Sale.tenant_id == tenant_id, Sale.store_id == store_id, Sale.is_cancelled == False)  # noqa: E712
