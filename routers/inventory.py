...
from app.models.core import Product, Batch, StockLevel
...
def create_product(payload: ProductCreate, tenant_id: str, db: Session = Depends(get_db)):
    existing = None
    if payload.barcode:
...
def list_products(tenant_id: str, db: Session = Depends(get_db)):
    return db.query(Product).filter(Product.tenant_id == tenant_id, Product.is_active == True).all()  # noqa: E712


@router.get("/products/barcode/{barcode}", response_model=ProductOut)
def get_product_by_barcode(barcode: str, tenant_id: str, db: Session = Depends(get_db)):
    """Used by the POS billing screen when cashier scans a barcode."""
    product = db.query(Product).filter(Product.tenant_id == tenant_id, Product.barcode == barcode).first()
    if not product:
...
def get_stock(store_id: str, product_id: str, db: Session = Depends(get_db)):
...
def receive_stock(
    ...
    # keep product's purchase price up to date for costing
...
def expiry_alerts(tenant_id: str, store_id: str, days_ahead: int = 7, db: Session = Depends(get_db)):
    return get_expiry_alerts(db, tenant_id, store_id, days_ahead)
